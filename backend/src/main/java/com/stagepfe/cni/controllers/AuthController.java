		...
		if (strRoles == null) {
			...
			strRoles.forEach(role -> {
				...
				case "agent":
					...
					break;
				case "agentA3T":
					Role agentA3TRole = roleRepository.findByName(ERole.AgentA3T)
							.orElseThrow(() -> new RuntimeException("Error: Role is not found."));
					roles.add(agentA3TRole);

					break;
				case "agentGN":
					Role agentGNRole = roleRepository.findByName(ERole.AgentGN)
							.orElseThrow(() -> new RuntimeException("Error: Role is not found."));
					roles.add(agentGNRole);
