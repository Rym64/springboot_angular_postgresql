...
@Component({
  ...
  styleUrls: ['./afficher-factures.component.scss']
})
...
export class AfficherFacturesComponent implements OnInit {
  ...
  getData(): void {
    this.webService.get("afficherFactures").subscribe(res => {
      let response = JSON.parse(JSON.stringify(res))
      this.facturesList = response.data})
  }
 ...
 return {
  ...
  rechercherFacture(recherche){
   ...
   updateFacture(facture){  
    this.webService.setterFacture(facture);
    this.dialog.open(ModifierFactureComponent);
  }
